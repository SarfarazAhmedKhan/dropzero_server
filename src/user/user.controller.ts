...
export class UserController {
  ...
  @Post("/withdraw_claimed_token/:id")
  async withDrawClaimTokens(@Req() req: Request, @Res() res: Response) {
    try {
      const claimedToken = await this.userService.withDrawClaimTokens(req);
      res.status(200).send({
        responseCode: 201,
        responseMessage: constants.SUCCESS,
        result: claimedToken,
